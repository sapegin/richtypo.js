...
describe('English, recommended rules', () => {
	...
	it('should not wrap words with hyphen in nobr if parts are longer than 2 characters', () => {
		compare(
			rt(recommended, 'Paul-Löbe auto-da-fé and -g -w'),
			'Paul-Löbe auto-da-fé and -g__-w'
		);
	});
	...
	test('wrap abbreviations in <abbr>', () => {
		compare(
			...
			`<abbr>DOXIE</abbr> and <abbr>ONU</abbr> and__DaN`
		);
	});
